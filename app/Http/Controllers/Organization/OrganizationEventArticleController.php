...
class OrganizationEventArticleController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $article = $query->first();
        if (! $article) return response()->json(['message' => 'article_not_found'], 404);
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) throw ValidationException::withMessages($validator->messages()->toArray());
    ...
    public function update(Request $request)
    {
        ...
        if ($request->has('excerpt'))    $updates['excerpt'] = $request->excerpt;
        if ($request->filled('content')) $updates['content'] = $request->content;
        ...
        if (! empty($updates)) $article->update($updates);
    ...
    private function saveCoverImage(string $base64, string $orgRoute, string $eventRoute, string $slug): string
    {
        ...
        Image::make($base64)
            ->resize(1200, 630, function ($c) { $c->aspectRatio(); $c->upsize(); })
            ->encode('webp', 85)
